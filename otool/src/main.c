...
int				fill_file_struct(t_file *file, void *ptr, char *av)
{
	file->st_file = ptr;
	file->name = av;
...
int				open_arg(char *arg, t_file *file, int fd)
{
	struct stat	stats;
	void		*ptr;
	...
	if (fstat(fd, &stats) == 0)
	{
		...
		if (ptr != MAP_FAILED)
		{
			...
			file->ofile_size = file->size;
			if (fill_file_struct(file, ptr, arg) == 1)
				return (1);
	...
	if (file != NULL)
	{
		...
		if (argc > 1)
			while (*argv != '\0')
			{
				...
				fd != -1 ? open_arg(*argv, file, fd) : \
				ft_putendl("ERREUR OPEN");
				argv++;
			}
		else
		{
			ft_putendl("pls give me args !!!");
		}
	}
	free(file);
}
